...
mod tests {
    ...
    fn test_update() {
        ...
        assert_eq!(to_string(&command), "`ix = x");

        let command = command::update(func::new("k", vec!["x", "y"], "x"));
        assert_eq!(to_string(&command), "``kxy = x");
        ...
        assert_eq!(to_string(&command), "```sxyz = ``xz`yz");
    }
    ...
    fn test_eval() {
        ...
        assert_eq!(to_string(&command), "`ab");

        let command = command::eval(expr::l("x", "y"));
        assert_eq!(to_string(&command), "λx.y");
    }
    ...
    fn test_eval_head() {
        ...
        assert_eq!(to_string(&command), "!42 `ab");

        let command = command::eval_head(42, expr::l("x", "y"));
        assert_eq!(to_string(&command), "!42 λx.y");
    }
