...
pub fn format(expr: &Expr, result_path: &Path, next_path: &Path) -> Output {
    let (tokens, result_range, next_range) = tokens_with_range(expr, result_path, next_path);
    let string = tokens_to_string(&mut tokens);

    let mut meta = HashMap::new();
    meta.insert("result".to_string(), result_range);
    meta.insert("next".to_string(), next_range);
...
pub struct Output {
    pub string: String,
    pub meta: HashMap<String, Range>,
}

// ========================================================================== //

fn tokens_with_range<'a>(
    expr: &'a Expr,
    result_path: &Path,
    next_path: &Path,
) -> (Vec<Token<'a>>, Range) {
    ...
    let args_tokens = Vec::new();
    let next_range: Range;
    for (index, arg) in args.into_iter().rev().enumerate() {
        let (need_range, next) = match &next_path {
            Path::Arg(i, next) if *i == index => {
                let next: &Path = next;
                (true, Some(next))
            }
            _ => (false, None),
        ...
        if need_range {
            let (tokens, range) = tokens_with_range(arg, next);
        }

        args_tokens.push(value)
    }
...
mod tests {
    ...
    fn test_tokens_with_range() {
        ...
        //            ^
        let path = Path::Callee(0);
        let (tokens, range) = tokens_with_range(&expr, &path);
        assert_eq!(tokens, vec![Token::UpperIdent(Ident::Variable("a"))]);
        assert_eq!(range, Range { start: 4, end: 5 });
    }
