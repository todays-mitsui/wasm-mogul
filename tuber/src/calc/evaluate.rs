...
impl Eval {
    pub fn new(expr: Expr, context: Context) -> Self {
        Self {
            ...
            zipper: Zipper::new(expr),
            step: 0,
        }
    }

    pub fn next(&mut self) -> Option<EvalStep> {
        self.zipper.next(&self.context, &mut self.cursor)

        // match self.cursor {
        //     Cursor::LeftTree => {
        //         while let Expr::Apply { ref lhs, ref rhs } = self.expr.focus {
        //             self.expr.focus = **lhs;
        //             self.expr.stack.push(Zipper::new(**rhs));
        //         }

        //         let maybe_args = arity(&self.context, &self.expr.focus)
        //             .filter(|arity| *arity >= 1 || self.expr.stack.len() >= 1)
        //             .and_then(|arity| self.expr.stack.pop(arity));
...
impl Zipper {
    ...
    fn next(&self, context: &Context, mut cursor: &mut Cursor) -> Option<EvalStep> {
        match cursor {
            Cursor::LeftTree => {
                while let Expr::Apply { ref lhs, ref rhs } = self.focus {
                    self.focus = **lhs;
                    self.stack.push(Zipper::new(**rhs));
                }

                let maybe_args = arity(&context, &self.focus)
                    .filter(|arity| *arity >= 1 || self.stack.len() >= 1)
                    .and_then(|arity| self.stack.pop(arity));
                ...
                if let Some(args) = maybe_args {
                    ...
                    Some(EvalStep { expr: self.into() })
                } else {
                    // β簡約に必要な引数が無ければ、右側の枝の簡約を試みる
                    cursor = &mut Cursor::RightTree(0, Box::new(Cursor::LeftTree));
                    self.next(context, cursor)
                }
            }

            Cursor::RightTree(n, inner_cursor) => {
                match self.stack.nth(n) {
                    // スタックの n 番目の枝を取得し、その枝の簡約を試みる
                    Some(zipper) => match zipper.next(context, inner_cursor) {
                        Some(_) => Some(EvalStep { expr: self.into() }),

                        // n 番目の枝が簡約済みなら、n+1 番目の枝へ進む
                        None => {
                            cursor = &mut Cursor::RightTree(*n + 1, Box::new(Cursor::LeftTree));
                            self.next(context, cursor)
                        }
                    },
                    ...
                    None => {
                        cursor = &mut Cursor::Done;
                        None
                    }
                }
            }

            // Cursor が Done なら、簡約は済んでいる
            Cursor::Done => None,
        }
    }
}
...
impl Stack {
    ...
    fn push(&mut self, zipper: Zipper) {
        self.0.push(zipper);
    }

    fn pop(&mut self, n: usize) -> Option<Vec<Zipper>> {
        let length = self.len();
    ...
    fn nth(&mut self, n: usize) -> Option<&mut Zipper> {
        let len = self.0.len();
        if n >= len {
            None
        } else {
    ...
    fn iter(&self) -> impl Iterator<Item = &Zipper> {
        self.0.iter()
    }
}
...
enum Cursor {
    LeftTree,
    RightTree(usize, Box<Cursor>),
    Done,
}

impl From<&Zipper> for Expr {
    fn from(zipper: &Zipper) -> Self {
        let mut expr = zipper.focus.to_owned();
        for arg in zipper.stack.iter() {
...
mod tests {
    ...
    fn test_stack_pop() {
        let context = Context::new();
        let mut stack = Stack(vec![Zipper::new(expr::v("x")), Zipper::new(expr::v("y"))]);

        assert_eq!(stack.len(), 2);

        stack.push(Zipper::new(expr::v("z")));

        assert_eq!(stack.len(), 3);

        assert_eq!(
            stack.pop(2),
            Some(vec![Zipper::new(expr::v("z")), Zipper::new(expr::v("y"))])
        );

        assert_eq!(stack.len(), 1);

        assert_eq!(stack.pop(1), Some(vec![Zipper::new(expr::v("x"))]));
    ...
    fn test_stack_all() {
        ...
        assert_eq!(
            stack0.iter().collect::<Vec<&Zipper>>(),
            Vec::<&Zipper>::new()
        );
    }
    ...
    fn test_stack_nth() {
        ...
        assert_eq!(stack.nth(0), Some(&mut Zipper::new(expr::v("z"))));
        assert_eq!(stack.nth(1), Some(&mut Zipper::new(expr::v("y"))));
        assert_eq!(stack.nth(2), Some(&mut Zipper::new(expr::v("x"))));
